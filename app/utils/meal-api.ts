...
export const MealAPI = {
  ...
  searchMealsByName: async (query: string) => {
    try {
      // encodeURI example: chicken and rice => chicken%20and%20rice
  ...
  getMealsById: async (id: string) => {
    try {
      const response = await fetch(`${BASE_URL}/lookup.php?s=${id}`);
      const data = await response.json();
      // get the first meal in the array
  ...
  // get multiple random meals (default 6)
  ...
  transformMealData: (meal: any) => {
    ...
    // the api response is weird, so reformat it to make the data cleaner
    const ingredients = [];
    for (let i = 1; i <= 20; i++) {
